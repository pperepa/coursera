...
void LinkedList<T>::insertOrdered(const T &newData)
{
  ...
  // If the LinkedList is empty, insert the element right away.
  if (!head_)
  {
    head_ = insertNode;
    tail_ = insertNode;
  ...
  {
    Node *currNode = head_;
    // If the LinkedList is not empty, begin traversing.
    while (currNode)
    {
      // Insert at front.
      if (currNode == head_)
      {
        // First node's data is less than data to be inserted.
        if (insertNode->data > currNode->data)
        {
          currNode = currNode->next;
        }
        // First node's data is greater than or equal to thedata to be inserted.
        else if (insertNode->data <= currNode->data)
        {
          insertNode->prev = currNode->prev;
      ...
      {
        // Data to be inserted is the highest value in the entire linked list.
        if (insertNode->data > currNode->data)
        {
        ...
        // Data to be inserted is less than the current data value.
        else if (insertNode->data < currNode->data)
        {
          insertNode->prev = currNode->prev;
          currNode->prev = insertNode;
        ...
        // Data to be inserted is equal to the current data value.
        else
        {
          insertNode->next = currNode->next;
          insertNode->prev = currNode;
      ...
      {
        // Move on to next node if the data value is greater than the current node's.
        if (insertNode->data > currNode->data)
        {
          currNode = currNode->next;
        }
        // Insert at previous node if a higher or equal data value is encountered.
        else if (insertNode->data < currNode->data)
        {
          currNode->prev->next = insertNode;
          insertNode->prev = currNode->prev;
          insertNode->next = currNode;
          currNode->prev = insertNode;
          size_ += 1;
        ...
        // Insertion data value is equal to current data value.
        else
        {
          insertNode->prev = currNode;
          insertNode->next = currNode->next;
